# Call from existing hosted zone and register records
data "aws_route53_zone" "kdigital_shop" {
  name = "kdigital.shop"
}
output "aws_route53_zone" {
  value = data.aws_route53_zone.kdigital_shop.id
}
resource "aws_route53_record" "server1-a" {
  zone_id = data.aws_route53_zone.kdigital_shop.zone_id
...
resource "aws_route53_record" "www-cname-d" {
  zone_id = data.aws_route53_zone.kdigital_shop.zone_id
  name    = "nfs.kdigital.shop"
  type    = "CNAME"
  ttl     = "300"
  records = ["k8s-n3.kdigital.shop"]
}
/*
/New hosted_zone and add records
